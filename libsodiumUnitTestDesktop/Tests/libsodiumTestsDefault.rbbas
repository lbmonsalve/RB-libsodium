#tag Class
Protected Class libsodiumTestsDefault
Inherits TestGroup
	#tag Method, Flags = &h0
		Sub HashTest()
		  ...
		  Assert.AreSame hash, EncodeHex(libsodium.GenericHash("Hello, world!", DecodeHex(key))), "GenericHash"
		  ...
		  Assert.AreEqual m, h, "SHA256 test_message"
		  
		  h = libsodium.SHA256("")
		  m = DecodeHex(sha256_empty)
		  Assert.AreEqual m, h, "SHA256 """""
		  ...
		  Assert.AreEqual m, h, "SHA512 test_message"
		  
		  // the empty string
		  h = libsodium.SHA512("")
		  m = DecodeHex(sha512_empty)
		  Assert.AreEqual m, h, "SHA512 """""
		  ...
		  Assert.AreEqual m, h, "blake2b test_message1"
		  
		  // the empty string
		  h = libsodium.GenericHash("")
		  m = DecodeHex(blake2b_empty)
		  Assert.AreEqual m, h, "blake2b """""
		End Sub
	#tag EndMethod

	#tag Method, Flags = &h0
		Sub KeyStream_ChaCha20Test()
		  ...
		  Assert.IsTrue(libsodium.StrComp(prefix, output))
	...
	#tag Method, Flags = &h0
		Sub KeyStream_XSalsa20Test()
		  ...
		  Assert.IsTrue(libsodium.StrComp(ciphertext, output))
	...
	#tag Method, Flags = &h0
		Sub PasswordTest()
		  ...
		  Assert.IsTrue(pass.VerifyHash(libsodium.DecodeHex(argon2), pass.ALG_ARGON2))
		  Assert.IsTrue(pass.VerifyHash(libsodium.DecodeHex(scrypt), pass.ALG_SCRYPT))
		  
		  Dim skey As libsodium.SKI.SecretKey
		  skey = skey.Import(TestSecretKey, pass)
		  
		  Assert.IsTrue(libsodium.StrComp(skey.Value, libsodium.DecodeHex(seckey)))
		  TestSKIEncrypt(skey)
		  Dim sigk As New libsodium.PKI.SigningKey(pass, libsodium.DecodeHex(salt))
		  
		  Assert.IsTrue(libsodium.StrComp(sigk.PrivateKey, libsodium.DecodeHex(sigskey)))
		  Assert.IsTrue(libsodium.StrComp(sigk.PublicKey, libsodium.DecodeHex(sigpkey)))
		  ...
		  Assert.IsTrue(libsodium.StrComp(enck.PrivateKey, libsodium.DecodeHex(privkey)))
		  Assert.IsTrue(libsodium.StrComp(enck.PublicKey, libsodium.DecodeHex(pubkey)))
	...
	#tag Method, Flags = &h0
		Sub PKIExchangeTest()
		  ...
		  alice = libsodium.PKI.EncryptionKey.Derive(DecodeHex(ALICE_SK))
		  Assert.IsTrue(alice.PublicKey = DecodeHex(ALICE_PK))
		  
		  Dim bob As libsodium.PKI.EncryptionKey
		  bob = libsodium.PKI.EncryptionKey.Derive(DecodeHex(BOB_SK))
		  Assert.IsTrue(bob.PublicKey = DecodeHex(BOB_PK))
		  ...
		  Assert.IsTrue(EncodeHex(k) = BOB_ALICE_SECRET)
		End Sub
	#tag EndMethod

	#tag Method, Flags = &h0
		Sub PKIForeignKeyTest()
	...
	#tag Method, Flags = &h0
		Sub PKISignTest()
		  ...
		  Assert.IsTrue(libsodium.PKI.VerifyData(sig, senderkey.PublicKey) <> Nil)
		  ...
		  Try
		    Do Until tis.EOF
		      ...
		      k = k.Derive(skey)
		      Assert.IsTrue(k.PublicKey = pkey)
		      Assert.IsTrue(k.PrivateKey = skey)
		      Assert.IsTrue(libsodium.PKI.VerifyData(msg, New libsodium.PKI.ForeignKey(k), sig))
		    Loop
		  Finally
		    tis.Close
		  End Try
	...
	#tag Method, Flags = &h0
		Sub SecureMemoryTest()
		  Dim data As SecureMemoryBlock = "Hello, world!"
		  data = New SecureMemoryBlock(64)
		  data.ZeroFill
		  
		  data.BooleanValue(0) = True
		  Assert.IsTrue(data.BooleanValue(0))
		  data.ZeroFill
		  
		  data.ByteValue(0) = 123
		  Assert.IsTrue(data.ByteValue(0) = 123)
		  data.ZeroFill
		  
		  data.ColorValue(0, 32) = &cFFFFFF00
		  Assert.IsTrue(data.ColorValue(0, 32) = &cFFFFFF00)
		  data.ZeroFill
		  
		  data.CString(0) = "123"
		  Assert.IsTrue(data.CString(0) = "123")
		  data.ZeroFill
		  
		  data.CurrencyValue(0) = 123.456
		  Assert.IsTrue(data.CurrencyValue(0) = 123.456)
		  data.ZeroFill
		  
		  data.DoubleValue(0) = 123.456
		  Assert.IsTrue(data.DoubleValue(0) = 123.456)
		  data.ZeroFill
		  
		  data.Int16Value(0) = 123
		  Assert.IsTrue(data.Int16Value(0) = 123)
		  data.ZeroFill
		  
		  data.Int32Value(0) = 123
		  Assert.IsTrue(data.Int32Value(0) = 123)
		  data.ZeroFill
		  
		  data.Int64Value(0) = 123
		  Assert.IsTrue(data.Int64Value(0) = 123)
		  data.ZeroFill
		  
		  data.Long(0) = 123
		  Assert.IsTrue(data.Long(0) = 123)
		  data.ZeroFill
		  
		  data.PString(0) = "123"
		  Assert.IsTrue(data.PString(0) = "123")
		  data.ZeroFill
		  
		  data.SingleValue(0) = 123.4
		  Assert.IsTrue(data.SingleValue(0) > 123.39)
		  Assert.IsTrue(data.SingleValue(0) < 123.41)
		  data.ZeroFill
		  
		  data.StringValue(0, 5) = "123.4"
		  Assert.IsTrue(data.StringValue(0, 5) = "123.4")
		  data.ZeroFill
		  
		  data.UInt16Value(0) = 123
		  Assert.IsTrue(data.UInt16Value(0) = 123)
		  data.ZeroFill
		  
		  data.UInt32Value(0) = 123
		  Assert.IsTrue(data.UInt32Value(0) = 123)
		  data.ZeroFill
		  
		  data.UInt64Value(0) = 123
		  Assert.IsTrue(data.UInt64Value(0) = 123)
		  data.ZeroFill
		  
		  data.UInt8Value(0) = 123
		  Assert.IsTrue(data.UInt8Value(0) = 123)
		  data.ZeroFill
		  
		  data.WString(0) = "123"
		  Assert.IsTrue(data.WString(0) = "123")
		End Sub
	#tag EndMethod
	...
	#tag Method, Flags = &h0
		Sub SKIMACTest()
		  ...
		  Assert.IsTrue(libsodium.SKI.VerifyMAC(sig, msg, key))
		End Sub
	#tag EndMethod

	#tag Method, Flags = &h0
		Sub TestPKIEncrypt(SenderKey As libsodium.PKI.EncryptionKey = Nil, Nonce As MemoryBlock = Nil)
		  ...
		  For i As Integer = 0 To UBound(msgs)
		    n = libsodium.IncrementNonce(n)
		    Assert.IsTrue(n <> nonce)
		    Assert.IsTrue(libsodium.PKI.DecryptData(msgs(i), senderkey.PublicKey, recipkey, n) = msg1)
		  Next
		End Sub
	#tag EndMethod

	#tag Method, Flags = &h0
		Sub TestSKIEncrypt(SecretKey As libsodium.SKI.SecretKey = Nil, Nonce As MemoryBlock = Nil)
	...
	#tag Method, Flags = &h0
		Sub UtilsTest()
		  ...
		  Assert.IsTrue(hex = EncodeHex(msg))
		  Assert.IsTrue(msg = DecodeHex(hex))
		  
		  Assert.IsTrue(libsodium.StrComp(msg, msg))
		  Assert.IsTrue(Not libsodium.StrComp(msg, ConvertEncoding(msg, Encodings.UTF16)))
		  Assert.IsTrue(Not libsodium.StrComp(msg, "adfsdfsdfsdf"))
		  ...
		  Assert.IsTrue(libsodium.CompareNonce(n, libsodium.IncrementNonce(n)) <> 0)
		End Sub
	#tag EndMethod
